...
public class FormBarang extends JFrame {
    ...
    void loadData() {
        try {
            ...
            while (rs.next()) {
                ...
                stokBarang = rs.getString("stok_barang");
                hargaBarang = rs.getString("harga_barang");
                String[] data = {kodeBarang, namaBarang, stokBarang, hargaBarang};
                df.addRow(data);
            }
            rs.close();
            cn.close();
    ...
    void simpanData(Barang B) {
        try {
            Connection cn = new connecDB().getConnect();
            String sql = "INSERT INTO barang (kode_barang, nama_barang, stok_barang, harga_barang) VALUES (?, ?, ?, ?)";
            PreparedStatement ps = cn.prepareStatement(sql);
            ps.setString(1, B.getKodeBarang());
            ps.setString(2, B.getNamaBarang());
            ps.setString(3, B.getStokBarang());
            ...
            String[] data = {B.getKodeBarang(), B.getNamaBarang(), B.getStokBarang(), B.getHargaBarang()};
            df.addRow(data);
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
    ...
    void ubahData(Barang B, String kodeBarang) {
        try {
            Connection cn = new connecDB().getConnect();
            String sql = "UPDATE barang SET kode_barang=?, nama_barang=?, stok_barang=?, harga_barang=? WHERE kode_barang=?";
            PreparedStatement ps = cn.prepareStatement(sql);
            ps.setString(1, B.getKodeBarang());
            ps.setString(2, B.getNamaBarang());
            ps.setString(3, B.getStokBarang());
    ...
    private void btaddAksi(ActionEvent evt) {
        ...
        B.setStokBarang(txstok.getText());
